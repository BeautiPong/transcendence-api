...
websocket_urlpatterns = [
    ...
    path('ws/match/<str:waiting_room>/<str:room_name>/<str:host>/', MatchingConsumer.as_asgi()),  # 특정 대기실과 방 이름을 포함한 매칭 연결
    path('ws/game/online/<str:room_name>/', GameConsumer.as_asgi()),  # 게임 관련 WebSocket 연결
    path('ws/game/offline/<str:user1>/<str:user2>/', OfflineConsumer.as_asgi()),  # 오프라인 게임 관련 WebSocket 연결
    path('ws/game/offline/<str:user1>/<str:user2>/<str:user3>/<str:user4>/', OfflineConsumer.as_asgi()),  # 오프라인 게임 관련 WebSocket 연결
